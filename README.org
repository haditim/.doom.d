...
** Reload Doom Emacs
Using =M-x doom/reload= inside Emacs (it sometimes does not load all newly installed packages, safer is to =>) or =~/.emacs.d/bin/doom sync= in terminal. This assures that the literal ~config.org~ gets processed.
